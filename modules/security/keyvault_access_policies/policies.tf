...
module "backup_vaults" {
  ...
  object_id = coalesce(
    try(var.resources.backup_vaults[each.value.lz_key][each.value.backup_vault_key].principal_id, null),
    try(var.resources.backup_vaults[var.client_config.landingzone_key][each.value.backup_vault_key].rbac_id, null),
  )
}