...
resource "azurerm_kubernetes_cluster" "aks" {
  ...
  default_node_pool {
    ...
    auto_scaling_enabled          = try(var.settings.default_node_pool.enable_auto_scaling, false)
    host_encryption_enabled       = try(var.settings.default_node_pool.enable_host_encryption, false)
    node_public_ip_enabled        = try(var.settings.default_node_pool.enable_node_public_ip, false)
  ...
  automatic_upgrade_channel  = try(var.settings.automatic_channel_upgrade, null)
  ...
  dynamic "api_server_access_profile" {
    ...
    content {
      ...
      virtual_network_integration_enabled = try(api_server_access_profile.value.vnet_integration_enabled, false)
    }
  }
  ...
  #  public_network_access_enabled       = try(var.settings.public_network_access_enabled, true)  # depricated as of v4.0
  ...
  dynamic "web_app_routing" {
    ...
    content {
      dns_zone_ids = try(web_app_routing.value.dns_zone_id, null)
    }
  }
}
...
resource "azurerm_kubernetes_cluster_node_pool" "nodepools" {
  ...
  auto_scaling_enabled    = try(each.value.enable_auto_scaling, false)
  host_encryption_enabled = try(each.value.enable_host_encryption, false)
  node_public_ip_enabled  = try(each.value.enable_node_public_ip, false)
