...
module "nat_gateways" {
  ...
  subnet_ids = [for subnet_key in each.value.subnet_keys :
    try(local.combined_objects_networking[try(each.value.vnet.lz_key, local.client_config.landingzone_key)][try(each.value.vnet.vnet_key, each.value.vnet_key)].subnets[try(subnet_key)].id, null)
  ]
}
